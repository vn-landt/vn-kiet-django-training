...
function processAndExtract() {
	...
	canvas.toBlob((blob) => {
		...
		let fileName = window.originalFileName;
		
		// Kiểm tra nếu fileName không hợp lệ hoặc là object, hãy reset về mặc định
		if (!fileName || typeof fileName !== 'string' || fileName.includes('blob')) {
			fileName = "extracted_" + Date.now() + ".jpg";
		}
		...
		if (window.extractMode === 'batch' && window.currentBatchCropIndex !== -1) {
			...
			if (typeof onImageCropped === 'function') {
				// Truyền fileName đã xử lý vào đây
				onImageCropped(blob, languagesStr, fileName, deleteDuration);
			} else {
